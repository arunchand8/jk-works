...
//       className="w-full bg-brand-dark flex flex-col items-center justify-center overflow-hidden py-26 md:py-6 lg:py-23 relative"
//     >
//       {/* ADVANCED SCROLL-KICKED KEYFRAMES */}
//       <style>{`
//         @keyframes split-divide {
...
//       {/* ==========================================================================
//          BOTTOM: PIPELINE PANEL (Reduced vertical gap here)
//          ========================================================================== */}
...
const WorkflowSkills = () => {
  ...
  return (
    <section
      id="workflow"
      // {/* 🛠️ Fixed invalid classes to py-20 md:py-24 lg:py-28 */}
      className="w-full bg-brand-dark flex flex-col items-center justify-center overflow-hidden py-20 md:py-24 lg:py-28 relative"
    >
      <style>{`
        @keyframes split-divide {
          0% { transform: translate(0px, 0px) scale(0.1); opacity: 0; }
        ...
        .divide-execution-layer { opacity: 0; }
        .active-orbit .master-orbit-track { animation: orbit-clockwise 25s linear infinite; animation-delay: 2s; }
        .active-orbit .child-node-fix { animation: node-counter-rotate 25s linear infinite; animation-delay: 2s; }
        .active-orbit .divide-execution-layer {
          --mx: var(--base-x);
          --my: var(--base-y);
          animation: split-divide 1.1s cubic-bezier(0.16, 1, 0.3, 1) forwards;
      ...
      >
        <div className="absolute text-center z-30 pointer-events-none select-none">
          <span className="font-nexa font-thin text-xs tracking-[0.4em] text-brand-gold/50 block mb-1 uppercase">My</span>
          <h2 className="font-nexa font-bold text-3xl md:text-4xl text-white tracking-widest uppercase">Workflow</h2>
          <p className="font-montserrat font-thin text-[10px] md:text-[13px] text-brand-gold tracking-wider mt-2">Tools i used to bring ideas to life</p>
        </div>
        ...
        <div className="absolute w-full h-full inset-0 flex items-center justify-center master-orbit-track">
          {tools.map((tool, index) => (
            ...
            >
              <div className="child-node-fix flex items-center justify-center">
                <div className={`w-16 h-16 md:w-20 md:h-20 lg:w-16 lg:h-16 rounded-full border flex items-center justify-center p-3.5 lg:p-3 ${tool.glow} transition-transform duration-300 hover:scale-110 cursor-pointer`}>
                  <img src={tool.img} alt={tool.alt} className="w-full h-full object-contain" />
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="w-full border border-brand-gold/10 rounded-xl p-6 md:p-8 bg-neutral-950/20 backdrop-blur-sm flex flex-col lg:flex-row items-center justify-center gap-y-6 gap-x-4 overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-y-5 gap-x-3 md:gap-x-4 w-full lg:w-auto">
            {sequence.map((item, index) => (
              ...
              >
                {index === 0 && (
                  <span className="text-brand-gold/80 text-xl font-sans font-medium select-none block md:hidden">↓</span>
                )}
                <div className={`w-11 h-11 md:w-13 md:h-13 rounded-full border flex items-center justify-center p-2.5 ${item.glow} shrink-0`}>
                  <img src={item.img} alt={item.alt} className="w-full h-full object-contain" />
                </div>
                <span className="text-brand-gold/80 text-xl md:text-2xl font-sans font-medium select-none md:block hidden">→</span>
                <span className="text-brand-gold/80 text-xl font-sans font-medium select-none block md:hidden">↓</span>
              </div>
            ))}
            ...
            >
              <h4 className="font-montserrat font-bold text-[10px] md:text-xs tracking-[0.15em] text-brand-gold uppercase leading-tight text-center">FINAL<br />OUTPUT</h4>
            </div>
          </div>
